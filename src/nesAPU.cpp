...
void nesAPU::cpuWrite(uint16_t addr, uint8_t data) {
    ...
    case 0x4003: // Pulse 1 Timer High / Length
        ...
        if (pulse1.enabled) pulse1.length_counter = length_table[(data & 0xF8) >> 3];
    ...
    case 0x4007: // Pulse 2 Timer High / Length
        ...
        if (pulse2.enabled) pulse2.length_counter = length_table[(data & 0xF8) >> 3];
    ...
    case 0x400B: // Triangle Timer High / Length
        ...
        if (triangle.enabled) triangle.length_counter = length_table[(data & 0xF8) >> 3];
    ...
    case 0x400F: // Noise Length
        if (noise.enabled) noise.length_counter = length_table[(data & 0xF8) >> 3];
    ...
    if (quarter_frame) {
        ...
        auto UpdateEnvelope = [](auto& chan) {
            if (chan.envelope_start) {
                ...
                chan.envelope_period = chan.envelope_decay_count + 1; // Not quite right but close
            } else {
                if (chan.envelope_decay_count > 0) {
                    chan.envelope_decay_count--;
                } else {
                    if (chan.halt) chan.envelope_decay_count = 15;
                }
            }
        };
...
double nesAPU::GetOutputSample() {
    ...
    // Mixer approximation (linear for now is simpler, though hardware is non-linear)
    ...
    if (p1_out > 0 || p2_out > 0) {
        pulse_out = 95.88 / ((8128.0 / (p1_out + p2_out)) + 100.0);
    }
    
    double tnd_out = 0.0;
    if (t_out > 0 || n_out > 0) {
        tnd_out = 159.79 / ((1.0 / (t_out / 8227.0 + n_out / 12241.0)) + 100.0);
    }
